...
void changeCanonState(Session session,String state) {
	...
	if(state == "MLHfanonOnly") session.canonLevel = CanonLevel.MLH_FANON_ONLY;
...
// 4/4/18 i'm trying to make this async instead of callback based so that AB will be saner.
 void checkEasterEgg(Session session){  //only yellow yard session uses 'that' because it needs to get back to the session context after doing easter egg.
	...
	if (getParameterByName("iNeedAGnosis", null) == "True") {
		GnosisStuck(session);
	}
}
	...
	if(session.session_id == 350) {
		...
		for (Player p in session.players) {
			if(p.aspect != Aspects.SPACE && p.aspect != Aspects.TIME) {
				...
				p.guardian.class_name = session.rand.pickFrom(SBURBClassManager.everything);
			} else{
				p.class_name = session.rand.pickFrom(SBURBClassManager.everything);
				p.guardian.class_name = session.rand.pickFrom(SBURBClassManager.everything);
			}
