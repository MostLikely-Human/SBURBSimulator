...
class GetTiger extends Scene{
	...
	String content() {
		...
		String ret = "" + getPlayersTitles(this.deadPlayersToGodTier) +
				" was always destined to take a Legendary Nap, and upon waking, become a God Tier. ";
		...
		if (withd != null && partyRollForLuck(withd) >
				25) { //MOST players in canon go god tier via sacrificial slab.
			for (num i = 0; i < withd.length; i++) {
				...
				if (p.hasLand()) {
					ret += " Upon being laid to rest on their QUEST BED on the " +
							p.land.name + ", the " + p.htmlTitle() +
							"'s body glows, and rises Skaiaward. ";
					ret += "On ${p
							.moonName}, their dream self takes over and gets a sweet new outfit to boot.  ";
				} else if (p.hasMoon()) {
					ret +=
					" You...aren't really sure how a real self made it to the SACRIFICIAL SLAB on ${p
							.moonName}, but there it is.  ";
					ret += "The " + p.htmlTitle() +
							" glows and ascends to the God Tiers with a sweet new outfit.  ";
				} else {
					p.godDestiny = false; //sorry, no land, no moon, you're fucked.
				}

				Fraymotif f = this.session.fraymotifCreator.makeFraymotif(
						rand, [p], 3); //first god tier fraymotif
				p.fraymotifs.add(f);
			...
			session.logger.debug(
					"We COULD have been on my quest bed, but random chance said no. ");
			return "";
		}
		...
		if (withoutd != null) {
			for (num i = 0; i < withoutd.length; i++) {
				...
				if (p.isDreamSelf && p.moon != null) {
					p.setStat(Stats.CURRENT_HEALTH, p.getStat(Stats.HEALTH));
					removeFromArray(this.session.afterLife.findClosesToRealSelf(p),
							this.session.afterLife.ghosts);
					//////session.logger.info("sacrificial slab: " + this.session.session_id);
					ret +=
							" Upon a wacky series of events leaving their corpse on their SACRIFICIAL SLAB on ${p
									.moonName}, the " + p.htmlTitle() +
									" glows and ascends to the God Tiers with a sweet new outfit.";
		...
		if (findClassPlayer(this.deadPlayersToGodTier, SBURBClassManager.PAGE) !=
				null) {
